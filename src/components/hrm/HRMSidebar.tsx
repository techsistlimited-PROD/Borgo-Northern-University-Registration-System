...
const menuItems: MenuItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { id: 'ess-payroll', label: 'Payroll & Tax', icon: DollarSign, path: '/hrm/ess/payroll' },
...
export default function HRMSidebar({ activePath, onNavigate }: Props) {
      ...
      <div key={item.id}>
        <button
          ...
          className={`w-full flex items-center justify-between space-x-3 px-4 py-3 rounded-xl transition-all ${
            ...
            isActive
              ? 'bg-light-lavender/30 text-deep-violet font-semibold border-l-4 border-soft-plum shadow-sm'
  ...
  return (
    <aside className="w-64 bg-gradient-to-b from-deep-violet to-soft-plum min-h-[calc(100vh-80px)] shadow-lg overflow-y-auto sticky top-20">
