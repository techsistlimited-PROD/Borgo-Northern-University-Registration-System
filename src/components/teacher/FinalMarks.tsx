...
export default function FinalMarks() {
  ...
  const [examDuration, setExamDuration] = useState<string>('4 hours')
  const [finalScores, setFinalScores] = useState<Record<string, FinalScoreEntry>>({})
  ...
  const handleLoadSection = async () => {
    ...
    try {
      ...
      const initialScores: Record<string, FinalScoreEntry> = {}
  ...
  const handleScoreChange = (studentId: string, newScore: number) => {
    if (newScore < 0 || newScore > 40) return
  ...
  const handleSaveScores = async () => {
    if (!selectedSection || Object.keys(finalScores).length === 0) {
      alert('Please load a section and enter scores first')
      return
    }

    // Validate all scores are entered
    const hasEmptyScores = Object.values(finalScores).some(entry => 
    ...
    try {
      ...
      alert('Final term marks saved and published successfully!')
    } catch (error) {
  ...
  const handleDownloadReport = () => {
    alert('Final term marks report downloaded successfully!')
  }
  ...
  const getStats = () => {
    ...
    const passCount = scoreValues.filter(score => (score / 40) * 100 >= 50).length
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-deep-plum">Final Term Marks Entry</h1>
          <p className="text-gray-600 mt-1">Enter and manage final examination marks (out of 40)</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Configure section and exam information for final term marks entry</CardDescription>
        </CardHeader>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Section {selectedSectionData.sectionName} • Final Examination
                  </p>
                  {examDate && (
                    <p className="text-sm text-gray-600">
      ...
      {Object.keys(finalScores).length > 0 && (
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center">
              <div>
                <CardTitle className="flex items-center space-x-2">
                  <Star className="w-5 h-5" />
                  <span>Final Term Marks Entry</span>
                </CardTitle>
                <CardDescription>
                  Enter final examination marks for each student (0-40 marks)
                </CardDescription>
              </div>
              
              <Button 
                onClick={handleSaveScores}
                disabled={isSaving || Object.values(finalScores).some(s => s.isFinalized)}
                className="flex items-center space-x-2"
              >
                {isSaving ? (
                  <>
          ...
          <CardContent>
              ...
              <table className="w-full border-collapse border border-gray-200">
                <thead>
                  <tr className="bg-gray-50">
                    ...
                    <th className="border border-gray-200 px-4 py-3 text-center font-medium">Marks (40)</th>
                ...
                <tbody>
                  {students.map((student) => {
                    const scoreData = finalScores[student.id]
                    if (!scoreData) return null
                    
                    const percentage = Math.round((scoreData.score / 40) * 100)
            ...
            {Object.values(finalScores).some(s => s.isFinalized) && (
              <div className="mt-4 p-4 bg-green-50 rounded-lg flex items-center space-x-2">
                <CheckCircle className="w-5 h-5 text-green-600" />
                <span className="text-green-800">
                  Final term marks have been published on {publishDate}
                </span>
              </div>
            )}
          </CardContent>
      ...
      {Object.keys(finalScores).length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Overview of final examination performance</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              {['A+', 'A', 'B+', 'B', 'C+', 'C', 'D', 'F'].map(grade => {
                const count = Object.values(finalScores).filter(score => 
                  getScoreGrade(score.score).grade === grade
                ).length
                ...
                return (
                  <div key={grade} className="text-center p-3 bg-gray-50 rounded-lg">
                    <p className="text-xl font-bold text-deep-plum">{count}</p>
        ...
        <Card>
          <CardContent className="py-20">
            <div className="text-center">
              ...
              <h3 className="text-xl font-semibold text-gray-700 mb-2">Select Section for Final Term Marks</h3>
              <p className="text-gray-500">Choose a section and configure exam details to begin entering final term marks</p>
            </div>
          </CardContent>
        </Card>
      )}
