...
import { canProceed, getStatusColor, computeTotalRequestedCredits } from '@/coe/utils/courseExemption'

interface CERequestTableProps {
  requests: CourseExemptionRequest[]
  selectedIds: string[]
  onSelect: (id: string) => void
  onSelectAll: (checked: boolean) => void
  onView: (request: CourseExemptionRequest) => void
  onEvaluate: (request: CourseExemptionRequest) => void
  onApprove: (id: string) => void
  onReject: (id: string) => void
  onApply: (id: string) => void
}

export default function CERequestTable({
  requests,
  selectedIds,
  onSelect,
  onSelectAll,
  ...
  const [sortBy, setSortBy] = useState<keyof CourseExemptionRequest>('updatedAt')
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('desc')
  const [currentPage, setCurrentPage] = useState(1)
  const itemsPerPage = 10

  const handleSort = (field: keyof CourseExemptionRequest) => {
    if (sortBy === field) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Exemption Request Queue</CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              {paginatedRequests.map(request => {
                ...
                const totalCredits = computeTotalRequestedCredits(request)
                ...
                return (
                  <tr key={request.id} className={`border-b hover:bg-gray-50 ${isBlocked ? 'bg-red-50' : ''}`}>
                    ...
                    <td className="p-3">
                      <Badge variant="outline">{totalCredits} cr</Badge>
                    </td>
                    <td className="p-3">
