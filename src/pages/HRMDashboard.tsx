...
import HRMDemoView from '@/components/hrm/HRMDemoView'
import { HRM_STATS } from '@/lib/hrmStatic'

type ActiveView = 'dashboard' | 'employees' | 'documents' | 'history' | 'recruitment' | 'attendance' | 'leave' | 'payroll' | 'performance' | 'training' | 'ess' | 'notices' | 'compliance' | 'demo' | 'other'
  ...
  const renderContent = () => {
    ...
    const content = (() => {
      switch (activeView) {
        ...
        case 'employees':
          return selectedEmployeeId ? <EmployeeProfile employeeId={selectedEmployeeId} onNavigate={handleNavigation} /> : <HRMEmployeeList onNavigate={handleNavigation} />
        case 'documents':
          return <HRMDocuments />
        case 'history':
          return <HRMHistory />
